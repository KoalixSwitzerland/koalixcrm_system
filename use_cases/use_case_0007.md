...
- Das `Product` (`kind = TRADING_GOOD` oder ein anderer angemessener `kind`-Wert für Mietgüter) existiert im aktiven Workspace mit `lifecycle_status = ACTIVE` und trägt mindestens eine `ProductVariant` mit `tracking_mode = SERIAL` ([ADR-0021](../adr/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md): „Das additive Feld `tracking_mode` ... wechselt von `Product` auf `ProductVariant`", [ADR-0009](../adr/0009-stock-domain-backbone.md) Amendment 2026-06-28).
- Mindestens eine `SerialUnit` dieser `ProductVariant` ist im Workspace vorhanden, trägt `condition_state ∈ {NEW, USED}` (nicht `DAMAGED`, nicht `IN_REPAIR`) und hat kein aktives `RentalAssignment` für den angefragten Zeitraum (`SerialUnit` trägt einen obligatorischen FK auf `ProductVariant`, [ADR-0012](../adr/0012-lifetime-batch-lot-serial-tracking.md) Amendment 2026-07-04).
- Der Benutzer ist authentifiziert und hat einen aktiven Workspace.
- Eine Kundenpartei (`Party`, [ADR-0001](../adr/0001-contact-and-party-data-model.md)) für den Mieter existiert im Workspace.
...
- Das Backend emittiert einen kompensierenden `StockMovement`-Eintrag mit `business_step = adjustment` und `compensates = FK auf den ursprünglichen geplanten rental_out-Event` ([ADR-0011](../adr/0011-stock-movements-and-event-log.md)).
...
- Der Mietvertrag wird bei der tatsächlichen Rückgabe durch einen `StockMovement`-Eintrag mit `business_step = rental_return` und `occurred_at = tatsächliches Rückgabedatum` abgeschlossen ([ADR-0011](../adr/0011-stock-movements-and-event-log.md)).
...
- Der `StockMovement`-Log für die `SerialUnit` ist als zeitliche Abfolge abfragbar und zeigt, wer die Einheit in welchem Zeitfenster hält ([ADR-0015](../adr/0015-unit-lifecycle-history.md): alle `StockMovement`-Zeilen mit `serial_unit_id = <id>`, sortiert nach `occurred_at`).
...
- [ ] Alle `StockMovement`-Einträge dieses Use Cases tragen `event_type = OBJECT_EVENT` ([ADR-0011](../adr/0011-stock-movements-and-event-log.md): „OBJECT_EVENT" für Lager- und Mietbewegungen).
- [ ] Die verwendeten `business_step`-Werte sind ausschließlich Werte aus dem in [ADR-0011](../adr/0011-stock-movements-and-event-log.md) definierten Wertebereich: `rental_out`, `rental_return`, `adjustment`, `commissioning`.
- [ ] Ein `StockMovement`-Eintrag mit `business_step = rental_out` trägt `qty = null`, da er ein Lifecycle-Planungsereignis ist und keine Mengenbewegung ([ADR-0011](../adr/0011-stock-movements-and-event-log.md): „ein Ereignis mit `qty = null` ist ein gültiges Lebenszyklus-Event").

---

## Architectural gaps surfaced

Die folgenden Lücken wurden beim Durcharbeiten dieses Use Cases gegenüber den ADRs 0009–0015 identifiziert. Sie sind als Kandidaten für ADR-Ergänzungen oder neue ADRs an `kxcrm-architect` eskaliert. Offene Fragen sind in `/app/koalixcrm-system/open_questions.md` unter [OQ-0010](../open_questions.md) bis [OQ-0014](../open_questions.md) eingetragen.

### Lücke 1 — CBV-Businessstep `rental_out` als Planungsereignis (soft) vs. physisches Ereignis ([OQ-0010](../open_questions.md))

[ADR-0011](../adr/0011-stock-movements-and-event-log.md) enthält `rental_out` und `rental_return` im `business_step`-Wertebereich. Die semantische Unterscheidung zwischen einem *geplanten* Mietbeginn (Angebot gespeichert, Gerät noch im Lager) und einem *physisch vollzogenen* Mietbeginn (Übergabe an Kunden stattgefunden) ist im CBV-Wertebereich von [ADR-0011](../adr/0011-stock-movements-and-event-log.md) nicht explizit definiert. GS1 EPCIS 2.0 CBV unterscheidet `bizStep` von `disposition`; eine `disposition = in_progress` würde den Planungszustand kennzeichnen. Ohne diese Unterscheidung ist aus dem Event-Log nicht ableitbar, ob ein `rental_out`-Eintrag eine bestätigte physische Übergabe oder eine Soft-Reservierung auf Angebotsebene beschreibt. [ADR-0011](../adr/0011-stock-movements-and-event-log.md) muss klären, ob `disposition` als Feld in `StockMovement` eingeführt wird oder ob ein separater Soft-Reservierungs-Businessstep (z. B. `planned_rental_out`) dem Wertebereich hinzugefügt wird.

### Lücke 2 — Zeitfensterbasierte ATP-Funktion fehlt ([OQ-0011](../open_questions.md))

[ADR-0010](../adr/0010-stock-states-and-reservations.md) definiert ATP als skalare Formel: `ATP = qty_on_hand − qty_booked − qty_reserved_for_document + qty_ordered`. Diese Formel ist für Mietanwendungen unzureichend, da Miet-ATP keine Mengenfrage ist, sondern eine Zeitfensterfrage: „Ist `SerialUnit` X im Intervall `[start, end]` frei?" Die skalare ATP-Formel liefert keine verwertbare Antwort auf diese Frage. Kein bestehendes ADR definiert eine zeitfensterbasierte Verfügbarkeitsfunktion `is_free(serial_unit, [start, end])` oder einen entsprechenden API-Endpunkt. Ohne diese Funktion ist der Verfügbarkeitskalender (BAC-1) nicht aus dem bestehenden Datenmodell ableitbar.

### Lücke 3 — Angebote partizipieren nicht am Reservierungsmechanismus ([OQ-0012](../open_questions.md))
...
### Lücke 4 — `RentalAssignment` hat kein zeitgebundenes Reservierungskonzept vor Übergabe ([OQ-0013](../open_questions.md))

[ADR-0013](../adr/0013-customer-held-rental-stock.md) definiert `RentalAssignment` als Bindung einer `SerialUnit` an einen aktiven Mietvertrag (`rental_start`, `return_due_date`). `RentalAssignment` entsteht jedoch erst bei physischer Übergabe (Mietbeginn). Zwischen Angebotsannahme und physischer Übergabe existiert ein Zeitraum, in dem die Einheit faktisch reserviert ist, aber weder ein aktives `RentalAssignment` noch eine formelle Bindung an die Einheit besteht — außer über `StockReservation`. Die Frage, ob `StockReservation` oder `RentalAssignment` die verbindliche Reservierungsquelle für den Verfügbarkeitskalender ist, ist zwischen [ADR-0010](../adr/0010-stock-states-and-reservations.md) und [ADR-0013](../adr/0013-customer-held-rental-stock.md) nicht abgestimmt. Eine doppelte Zählung (sowohl `StockReservation` als auch `RentalAssignment` blockieren denselben Slot) ist möglich.

### Lücke 5 — Halter-Zeitleiste als dedizierter Abfragepfad nicht definiert ([OQ-0014](../open_questions.md))

[ADR-0015](../adr/0015-unit-lifecycle-history.md) definiert den Abfragepfad „Vollständige Einheitenhistorie" als alle `StockMovement`-Zeilen mit `serial_unit_id = <id>`, sortiert nach `occurred_at`. Dieser Pfad enthält alle Ereignistypen: Lager-, Lifecycle- und Mietbewegungen gemischt. Eine explizite Projektion „Wer hat die Einheit in welchem Zeitfenster gehalten?" — d. h. eine Halter-Zeitleiste aus nur den `rental_out`/`rental_return`/`owner_type`-Paaren — ist kein definierter Abfragepfad in [ADR-0015](../adr/0015-unit-lifecycle-history.md). Für den Verfügbarkeitskalender (BAC-1) und die Rückgabehistorie muss diese Projektion entweder als dedizierter API-Endpunkt oder als materialisierte Sicht spezifiziert werden. Ohne diese Spezifikation implementieren verschiedene Entwickler inkonsistente Auswertungslogiken.

---

## Referenzen
- [ADR-0009](../adr/0009-stock-domain-backbone.md) — Lagerdomäne-Fundament, `Location`-Hierarchie, `tracking_mode`
- [ADR-0010](../adr/0010-stock-states-and-reservations.md) — `StockReservation`, ATP-Formel, generische Dokumentreferenz
- [ADR-0011](../adr/0011-stock-movements-and-event-log.md) — `StockMovement`-Log, EPCIS-CBV-`business_step`-Wertebereich
- [ADR-0012](../adr/0012-lifetime-batch-lot-serial-tracking.md) — `SerialUnit`, `condition_state`
- [ADR-0013](../adr/0013-customer-held-rental-stock.md) — `RentalAssignment`
- [ADR-0015](../adr/0015-unit-lifecycle-history.md) — Halter-/Einheitenhistorie via `StockMovement`-Log
- [ADR-0021](../adr/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md) — Bestandsschlüsselung auf `ProductVariant` statt `Product`
- [ADR-0019](../adr/0019-product-kind-invariants.md) — `kind`-Kontext des Mietguts (`TRADING_GOOD` oder anderer `kind`-Wert)
- [ADR-0001](../adr/0001-contact-and-party-data-model.md) — Mieter-`Party`
- [OQ-0010](../open_questions.md), [OQ-0011](../open_questions.md), [OQ-0012](../open_questions.md), [OQ-0013](../open_questions.md), [OQ-0014](../open_questions.md) — offene architektonische Lücken dieses Use Cases
...
- 2026-07-04: Lücke 6 ([ADR-0021](../adr/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md)-Restlücke) geschlossen: `SerialUnit`/`Batch` ([ADR-0012](../adr/0012-lifetime-batch-lot-serial-tracking.md) Amendment 2026-07-04), `StockBalance`/`StockReservation` ([ADR-0010](../adr/0010-stock-states-and-reservations.md) Amendment 2026-07-04) tragen nun einen obligatorischen FK auf `ProductVariant` statt `Product`; `RentalAssignment` bleibt feldseitig unverändert, da es keinen direkten `Product`-FK trägt und die Variantenschlüsselung transitiv über `SerialUnit`/`StockReservation` wirkt ([ADR-0013](../adr/0013-customer-held-rental-stock.md) Amendment 2026-07-04). Use-Case-Text und Diagramm entsprechend auf `ProductVariant` umgestellt: Verfügbarkeits-Endpunkt `GET /api/variants/{id}/serial-units/availability/` (statt `/api/products/{id}/...`), `StockReservation`-Insert trägt `variant` statt `product`. Lücke 6 aus der Gaps-Liste entfernt.
