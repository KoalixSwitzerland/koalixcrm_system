...
**ID:** UC-0012
**Bezug:** [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), [ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md), [ADR-0019](../09_architecture_decisions/0019-product-kind-invariants.md), [ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md), [ADR-0009](../09_architecture_decisions/0009-stock-domain-backbone.md), [ADR-0010](../09_architecture_decisions/0010-stock-states-and-reservations.md), [ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), [REQ-0015](../01_introduction_and_goals/requirements/REQ-0015.md); [ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md) Nachtrag 2026-07-04, [ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md) Amendments 2026-07-04 ([OQ-0019](../11_risks_and_technical_debt/open_questions.md), [OQ-0020](../11_risks_and_technical_debt/open_questions.md)) und [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md) Nachtrag 2026-07-04 (Komponenten-Variantenauflösung und As-Built-Eltern-Anker), je in Verbindung mit [ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md) §Ripple-Liste Komponenten-Variantenauflösung und As-Built-Anker
...
**Warum:** Kein bestehender Use Case durchläuft das Kitting-Modell ([ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), `kind = KIT`, `EXPLODE_ON_PICK`) oder das Montagemodell (`ProductionOrder`/`ProductionOrderComponent`, `kind = MANUFACTURED_GOOD`) end-to-end. Ohne diesen Durchlauf bleibt unbelegt, ob die Bestandsbindung über `StockReservation`, die Fertigstellungsbuchung über `TRANSFORMATION_EVENT`/`AGGREGATION_EVENT` und die durchgängige Varianten-Schlüsselung ([ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md), Amendment 2026-07-04) im Zusammenspiel funktionieren.
...
## Vorbedingungen
- Ein `Product` mit `kind = KIT` und `kit_mode = EXPLODE_ON_PICK` existiert im aktiven Workspace; seine `BillOfMaterials` trägt mindestens einen `BomItem` und einen aktuellen `BillOfMaterialsExplosion`-Snapshot (`bom_version` entspricht dem aktuellen `BillOfMaterials`-Stand, [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md)).
- Für jede Komponente der Kit-Explosion existiert ein ausreichender `OnHandRecord`-Bestand der jeweiligen Komponenten-`ProductVariant` ([ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md): „`OnHandRecord` FK → `ProductVariant` ist der autoritative Lager-Schlüssel").
- Ein `Product` mit `kind = MANUFACTURED_GOOD` existiert im aktiven Workspace mit einer `BillOfMaterials` ([ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md), [REQ-0015](../01_introduction_and_goals/requirements/REQ-0015.md)); ein `ProductionOrder` mit `status = DRAFT` referenziert diese `BillOfMaterials`.
- Für jede Komponente des Fertigungsauftrags ist ausreichender `OnHandRecord`-Bestand der jeweiligen Komponenten-`ProductVariant` verfügbar oder disponierbar (ATP-Formel, [ADR-0010](../09_architecture_decisions/0010-stock-states-and-reservations.md)).
...
- Beim Kit-Pick (Teil A) oder bei der `StockReservation`-Anlage bzw. physischen Entnahme (Teil B) stellt das Backend fest, dass die ATP-Formel (`ATP = qty_on_hand − qty_booked − qty_reserved_for_document + qty_ordered`, [ADR-0010](../09_architecture_decisions/0010-stock-states-and-reservations.md)) für mindestens eine Komponenten-`ProductVariant` nicht die angeforderte Menge deckt.
- Das Backend antwortet mit HTTP 422 und benennt die betroffene Komponenten-`ProductVariant` sowie die verfügbare Menge.
- Das Frontend zeigt die Unterdeckung an der betroffenen Komponentenzeile an; der Benutzer reduziert die Menge, wählt eine Alternativkomponente (`BomItem`, [ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md) AC-2) oder bricht den Vorgang ab.
...
- Beim Lesen von `GET /api/products/{kit_id}/bom-explosion/` stellt das Backend fest, dass kein `BillOfMaterialsExplosion`-Snapshot existiert oder dessen `bom_version` vom aktuellen `BillOfMaterials`-Stand abweicht ([ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), [OQ-0007](../11_risks_and_technical_debt/open_questions.md)).
...
- Das Backend weist die Anfrage mit HTTP 400 ab; die `ProductKindPolicy` ([ADR-0019](../09_architecture_decisions/0019-product-kind-invariants.md)) benennt den erlaubten Wertebereich `{MANUFACTURED_GOOD, KIT}` ([REQ-0015](../01_introduction_and_goals/requirements/REQ-0015.md) AC-1).
...
- Für jede kommissionierte Kit-Position (Teil A) existiert je Komponenten-`ProductVariant` genau ein `StockMovement`-Eintrag mit `event_type = OBJECT_EVENT`, `business_step = picking`, dessen `variant`-Feld die dreistufig aufgelöste Komponenten-`ProductVariant` obligatorisch trägt ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04, [OQ-0019](../11_risks_and_technical_debt/open_questions.md)); kein eigener `OnHandRecord`-Eintrag für das KIT-Produkt selbst entsteht (`EXPLODE_ON_PICK` erzeugt keinen Vorlagerbestand, [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md)).
- Jeder `ProductionOrderComponent`-Eintrag (Teil B) trägt die dreistufig aufgelöste Komponenten-`ProductVariant` obligatorisch im `variant`-Feld ([ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), Nachtrag 2026-07-04, [OQ-0019](../11_risks_and_technical_debt/open_questions.md)); dasselbe gilt für die zugehörige `StockReservation`.
- Für jeden abgeschlossenen `ProductionOrder` (Teil B) existiert genau ein `StockMovement`-Eintrag mit `event_type = TRANSFORMATION_EVENT` sowie mindestens ein `StockMovement`-Eintrag mit `event_type = AGGREGATION_EVENT`, der die As-Built-Provenienz des Fertigprodukts dokumentiert; alle `AGGREGATION_EVENT`-Zeilen desselben Fertigungsabschlusses tragen denselben obligatorischen `aggregation_group`-Wert ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04, [OQ-0020](../11_risks_and_technical_debt/open_questions.md)), unabhängig vom `tracking_mode` der Fertigprodukt-`ProductVariant`; zusätzlich ist `parent_serial_unit` gesetzt bei `tracking_mode = SERIAL` bzw. `parent_batch` bei `tracking_mode = BATCH`, während bei `tracking_mode = NONE` `aggregation_group` der alleinige Anker bleibt.
...
- Sämtliche in diesem Use Case erzeugten `OnHandRecord`-, `StockReservation`- und `StockMovement`-Zeilen sind auf die jeweilige `ProductVariant` geschlüsselt ([ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md), Amendment 2026-07-04), nicht auf `Product`; `StockMovement.variant` ist dabei durchgängig obligatorisch ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04, [OQ-0019](../11_risks_and_technical_debt/open_questions.md)).
...
## Referenzen
- [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md) — Kitting-Modell (`kind = KIT`, `EXPLODE_ON_PICK`), `ProductionOrderComponent.variant`
- [ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md) — `BillOfMaterials`/`BomItem`, `default_component_variant`
- [ADR-0019](../09_architecture_decisions/0019-product-kind-invariants.md) — `ProductKindPolicy`: erlaubter Wertebereich `{MANUFACTURED_GOOD, KIT}` für BOM/`ProductionOrder`
- [ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md) — durchgängige Varianten-Schlüsselung von `OnHandRecord`/`StockReservation`/`StockMovement`
- [ADR-0009](../09_architecture_decisions/0009-stock-domain-backbone.md) — Lagerdomäne-Fundament
- [ADR-0010](../09_architecture_decisions/0010-stock-states-and-reservations.md) — ATP-Formel, `StockReservation`
- [ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md) — `StockMovement`-Log, `TRANSFORMATION_EVENT`/`AGGREGATION_EVENT`, Komponenten-Variantenauflösung (Amendment 2026-07-04)
- [ADR-0015](../09_architecture_decisions/0015-unit-lifecycle-history.md) — Einheitenhistorie-Abfragepfad
- [REQ-0015](../01_introduction_and_goals/requirements/REQ-0015.md) — governing requirement
- [OQ-0007](../11_risks_and_technical_debt/open_questions.md), [OQ-0019](../11_risks_and_technical_debt/open_questions.md), [OQ-0020](../11_risks_and_technical_debt/open_questions.md) — bezogene/geschlossene offene Fragen
- [Glossar](../12_glossary/glossar.md) — Begriffsdefinitionen (`kind`, `ProductVariant`, `BillOfMaterials (BOM)`, `BomItem`)

---

## Änderungsprotokoll
- 2026-07-04: Lücke 1 ([OQ-0019](../11_risks_and_technical_debt/open_questions.md)) geschlossen: `BomItem` bleibt Product-gekeyt und trägt neu ein optionales, nicht-bindendes `default_component_variant`-Feld ([ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md), Nachtrag 2026-07-04); die verbindliche Komponenten-Variantenauflösung bindet erst am Buchungspunkt, dreistufig — (1) explizite Angabe im Request, (2) `BomItem.default_component_variant`, (3) die einzige `ProductVariant` des Komponenten-`Product` — mit HTTP 422 bei Nichtauflösbarkeit ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04). `ProductionOrderComponent.variant` ([ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), Nachtrag 2026-07-04) und `StockMovement.variant` ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04) sind dazu beide obligatorisch geworden; die von [ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md) offengelassene Ripple-Lücke (`[ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md)` fehlte in der Ripple-Liste) ist damit geschlossen. Hauptablauf (Teil A und Teil B) um die dreistufige Auflösung ergänzt; neuer Alternativablauf D für den 422-Fall; BAC-4 und BAC-5 entsprechend erweitert. Siehe [ADR-0006](../09_architecture_decisions/0006-sourcing-and-bill-of-materials.md), [ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md) und [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), je Nachtrag/Amendment 2026-07-04, sowie [ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md) §Ripple-Liste Komponenten-Variantenauflösung und As-Built-Anker.
- 2026-07-04: Lücke 2 ([OQ-0020](../11_risks_and_technical_debt/open_questions.md)) geschlossen: `StockMovement` trägt neu ein tracking-mode-unabhängiges Gruppierungsfeld `aggregation_group` (UUID, obligatorisch bei `event_type = AGGREGATION_EVENT`), das alle `AGGREGATION_EVENT`-Zeilen desselben Fertigungsabschlusses verknüpft, unabhängig davon, ob eine Eltern-`SerialUnit` existiert; ergänzend tragen die Zeilen `parent_serial_unit` (bei `tracking_mode = SERIAL`) bzw. `parent_batch` (bei `tracking_mode = BATCH`), bei `tracking_mode = NONE` bleibt `aggregation_group` alleiniger Anker ([ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md), Amendment 2026-07-04; [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), Nachtrag 2026-07-04). Hauptablauf (Teil B, Fertigungsabschluss) um die `aggregation_group`-Erzeugung ergänzt; BAC-3 auf Gruppierung nach `aggregation_group` statt Eltern-`SerialUnit` umgestellt. Siehe [ADR-0011](../09_architecture_decisions/0011-stock-movements-and-event-log.md) und [ADR-0014](../09_architecture_decisions/0014-assembly-kitting-and-split-stock.md), je Amendment/Nachtrag 2026-07-04.
- 2026-07-04: Ersterstellung. Schließt die Validierungslücke, dass kein Use Case das Kitting-Modell (`EXPLODE_ON_PICK`) und das Montagemodell (`ProductionOrder`/`ProductionOrderComponent`, `TRANSFORMATION_EVENT` + `AGGREGATION_EVENT`) end-to-end unter durchgängiger Varianten-Schlüsselung ([ADR-0021](../09_architecture_decisions/0021-produkt-variantengranularitaet-topologie-schluesselung-attributkaskade.md), Amendment 2026-07-04) durchspielt. Lücke 1 ([OQ-0019](../11_risks_and_technical_debt/open_questions.md)) und Lücke 2 ([OQ-0020](../11_risks_and_technical_debt/open_questions.md)) an `kxcrm-architect` eskaliert.
