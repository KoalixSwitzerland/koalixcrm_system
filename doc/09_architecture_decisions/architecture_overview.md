...
| ADR-0016 | Identifier-Registry und Barcode-Auflösung                                     | Accepted | Kanonischer Endpunkt `POST /api/v1/scan/resolve`; zweistufige Auflösung: GS1 AI-Parsing zuerst (GTIN→ProductVariant, SSCC→HandlingUnit, SGTIN/GIAI→SerialUnit, GLN→Location), danach Freitext-Abgleich (inkl. SKU→ProductVariant); GS1 schlägt Freitext; Freitext-Multi-Hit → HTTP 409; workspace-scoped außer `ProductVariant.gtin` (katalogweit, Nachtrag 2026-07-04). |
| ADR-0017 | GoodsReceipt als Prozess-Aggregat                                             | Accepted | `GoodsReceipt`-Aggregat mit `status ∈ {DRAFT, IN_PROGRESS, COMPLETED, CANCELLED}` und `GoodsReceiptLine` (FK → `ProductVariant`, obligatorisch, Nachtrag 2026-07-04; `line_status ∈ {PENDING, CONFIRMED, MISMATCHED}`); COMPLETED erzeugt synchrone `StockMovement`-Buchungen je Position (`business_step=receiving`); Ingestion via strukturiertem JSON-Payload; OCR/EDI als externe Adapter; Put-Away-Strategie offen (OQ-0015). |
...
| ADR-0019 | Produkt-`kind`-Invarianten und Gating abhängiger Objekte                      | Accepted | Konsolidiert alle kind-gekoppelten Gating-Regeln (ServiceProfile, BillOfMaterials, ProductionOrder, Lagerentitäten, tracking_mode) in einer kanonischen Matrix; definiert das Sperr-Set für kind-Unveränderlichkeit; schreibt `ProductKindPolicy` als einzige Durchsetzungskomponente vor. |
| ADR-0020 | Gemeinsame deklarative Attributvalidierung zwischen Backend und Frontend       | Accepted | Validierungsregeln für feldübergreifende, konditionale Attribute werden als serialisierbare Daten im Backend gespeichert und über die REST-API ausgeliefert; das Backend ist autoritativer Enforcer; das Frontend wertet dieselben Regeln für live-UX aus; eine gemeinsame Konformitätstestsuite verhindert Divergenz strukturell. |
| ADR-0021 | Produkt-Variantengranularität — Topologie, Schlüsselung und Attribut-Vererbungskaskade | Accepted | Fixiert die 3-Ebenen-Topologie (`ProductFamily` optional → `Product` → `ProductVariant` ≥1, FK auf `Product`), Schlüsselung (SKU/GTIN/Preis/Lager auf Variant; Kind/Klassifizierung/BOM/ServiceProfile auf Product), Attributkaskade (Varianten-Override → Produkt-Wert → Familie/AttributeSet-Standard, Lesezeit, Option A mit nullable `variant_id`); korrigiert ADR-0003, ADR-0004, ADR-0005, ADR-0006, ADR-0009, ADR-0010, ADR-0011, ADR-0012, ADR-0013, ADR-0014, ADR-0016 und ADR-0017 via Amendment (Bestands-, Chargen-, Serien-, Reservierungs-, Komponenten-, Wareneingangs- und Identifier-Auflösungs-Variantenschlüsselung durchgängig auf `ProductVariant`; As-Built-Eltern-Anker für alle `tracking_mode`-Werte). |
