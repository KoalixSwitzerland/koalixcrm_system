...
| ADR-0021 | Produkt-Variantengranularität — Topologie, Schlüsselung und Attribut-Vererbungskaskade | Proposed | Fixiert die 3-Ebenen-Topologie (`ProductFamily` optional → `Product` → `ProductVariant` ≥1, FK auf `Product`), Schlüsselung (SKU/GTIN/Preis/Lager auf Variant; Kind/Klassifizierung/BOM/ServiceProfile auf Product), Attributkaskade (Varianten-Override → Produkt-Wert → Familie/AttributeSet-Standard, Lesezeit, Option A mit nullable `variant_id`); korrigiert ADR-0003, ADR-0004, ADR-0005 und ADR-0009 via Amendment. |
