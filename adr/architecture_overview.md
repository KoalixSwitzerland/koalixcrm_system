...
| ADR-0019 | Produkt-`kind`-Invarianten und Gating abhängiger Objekte                      | Proposed | Konsolidiert alle kind-gekoppelten Gating-Regeln (ServiceProfile, BillOfMaterials, ProductionOrder, Lagerentitäten, tracking_mode) in einer kanonischen Matrix; definiert das Sperr-Set für kind-Unveränderlichkeit; schreibt `ProductKindPolicy` als einzige Durchsetzungskomponente vor. |
| ADR-0020 | Gemeinsame deklarative Attributvalidierung zwischen Backend und Frontend       | Proposed | Validierungsregeln für feldübergreifende, konditionale Attribute werden als serialisierbare Daten im Backend gespeichert und über die REST-API ausgeliefert; das Backend ist autoritativer Enforcer; das Frontend wertet dieselben Regeln für live-UX aus; eine gemeinsame Konformitätstestsuite verhindert Divergenz strukturell. |
| ADR-0021 | Produkt-Variantengranularität — Topologie, Schlüsselung und Attribut-Vererbungskaskade | Proposed | Fixiert die 3-Ebenen-Topologie (`ProductFamily` optional → `Product` → `ProductVariant` ≥1, FK auf `Product`), Schlüsselung (SKU/GTIN/Preis/Lager auf Variant; Kind/Klassifizierung/BOM/ServiceProfile auf Product), Attributkaskade (Varianten-Override → Produkt-Wert → Familie/AttributeSet-Standard, Lesezeit, Option A mit nullable `variant_id`); korrigiert ADR-0003, ADR-0004, ADR-0005, ADR-0009, ADR-0010, ADR-0012 und ADR-0013 via Amendment (Bestands-, Chargen-, Serien- und Reservierungsschlüsselung durchgängig auf `ProductVariant`). |
