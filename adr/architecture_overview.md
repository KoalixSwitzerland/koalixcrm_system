...
| ADR-0021 | Produkt-Variantengranularität — Topologie, Schlüsselung und Attribut-Vererbungskaskade | Accepted | Fixiert die 3-Ebenen-Topologie (`ProductFamily` optional → `Product` → `ProductVariant` ≥1, FK auf `Product`), Schlüsselung (SKU/GTIN/Preis/Lager auf Variant; Kind/Klassifizierung/BOM/ServiceProfile auf Product), Attributkaskade (Varianten-Override → Produkt-Wert → Familie/AttributeSet-Standard, Lesezeit, Option A mit nullable `variant_id`); korrigiert ADR-0003, ADR-0004, ADR-0005, ADR-0006, ADR-0009, ADR-0010, ADR-0011, ADR-0012, ADR-0013 und ADR-0014 via Amendment (Bestands-, Chargen-, Serien-, Reservierungs- und Komponenten-Variantenschlüsselung durchgängig auf `ProductVariant`; As-Built-Eltern-Anker für alle `tracking_mode`-Werte). |
